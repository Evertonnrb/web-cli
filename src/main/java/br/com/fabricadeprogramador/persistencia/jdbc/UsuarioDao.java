...
public class UsuarioDao {
	...
	public void alterar(Usuario usu) {
		...
		// try (){}==esse try e tipo um if (mais inteligente é auto closeble
		// repare ele encerra a conxao auto)
	...
	public Usuario autenticar(Usuario usuConsulta) {
		...
		try (PreparedStatement prep = con.prepareStatement(sql)) {
			prep.setString(1, usuConsulta.getLogin());
			prep.setString(2, usuConsulta.getSenha());
			ResultSet resul = prep.executeQuery();
			...
			if (resul.next()) {
				...
				usu.setId(resul.getInt("id"));
				usu.setNome(resul.getString("nome"));
				usu.setLogin(resul.getString("senha"));
